...
export const jsBoostWidgets = ((selector = "", args = {}) => {
    ...
    var flyToLeftCtrl  = null;
    var flyToRightCtrl = null;

    function flyToRight(_element_left = "", _element_right = "") {
        clearInterval(flyToLeftCtrl);
        clearInterval(flyToRightCtrl);
        
        let _elementLeft  = document.getElementById(_element_left.replace("#", ""));
        let _elementRight = document.getElementById(_element_right.replace("#", ""));
        
        let _stylesLeft  = getStyles(_elementLeft);
        let _stylesRight = getStyles(_elementRight);
        
        let _left        = parseInt(_stylesLeft.styles.marginLeft);
        let _right       = parseInt(_stylesRight.styles.marginRight);
        
        let _topLeft     = parseInt(_stylesLeft.styles.marginTop);
        let _topRight    = parseInt(_stylesRight.styles.marginTop);

        console.log(_stylesRight);

        // Right transition
        _elementRight.style.transition = "all 100ms ease-out";
        _elementRight.style.position   = "fixed";

        var flyToRightCtrl = setInterval(function(){
            ...
            if(_right <= 10) {
                ...
                _right -= 10;
                _elementRight.style.right = _right + "px";
                _elementRight.style.top   = _right + "px";
            }

        }, 1);

        // Left transition
        _elementLeft.style.transition = "all 100ms ease-out";
        _elementLeft.style.position   = "absolute";

        flyToLeftCtrl = setInterval(function(){
            ...
            if(_left <= 10) {
                ...
                _elementLeft.style.left = _left + "px";
                _elementLeft.style.top  = _left + "px";
            }
        ...
        fadeCtrl = setInterval(function(){
            if((_opacity == 100)) {
                ...
                if(parseInt(args.timeout) > 0) {
                    ...
                    setTimeout(function(){
                        jsBoostWidgets(selector).fadeOut();
    ...
    function fade() {
        if(args.effect == "fadein") {
            ...
            .addEventListener("click", function(){
                jsBoostWidgets(args.element, args).fadeIn();
            });
        } else if(args.effect == "fadeout") {
            document.getElementById(selector.replace("#", ""))
            .addEventListener("click", function(){
                jsBoostWidgets(args.element).fadeOut();
            });
        } else {
            console.error("Erro on args.effect: function jsBoostWidgetsfade(), received:", args.effect);
        }
    }
    ...
    function retract() {
        if(args.command == "show") {
            ...
            .addEventListener("click", function(event){
                ...
                jsBoostWidgets(args.element, args).show();
            });
        } else if(args.command == "hide") {
            document.getElementById(selector.replace("#", ""))
            .addEventListener("click", function(event){
                event.preventDefault();
                jsBoostWidgets(args.element).hide();
            });
        } else {
            console.error("Erro on args.command: function jsBoostWidgetsretract(), received:", args.command);
        }
    }
    ...
    function autoModalClose(params = {}) {
        setTimeout(function(){
            jsBoostWidgets(params.sel).modalClose();
                                ...
                                modalCtrl = setInterval(function(){
                                    ...
                                    if(_morewidth <= _styles.width) {
                                        ...
                                        if(parseInt(args.timeout) > 0) {
                                            ...
                                            setTimeout(function(){
                                                jsBoostWidgets(selector).modalClose();
                                            }, parseInt(args.timeout));
    ...
    function modal() {
        if(args.command == "open") {
            ...
            .addEventListener("click", function(event){
                event.preventDefault();
                jsBoostWidgets(args.element, args).modalOpen();
            });
        } else if(args.command == "close") {
            document.getElementById(selector.replace("#", ""))
            .addEventListener("click", function(event){
                event.preventDefault();
                jsBoostWidgets(args.element, args).modalClose();
            });
        } else {
            console.error("Erro on args.command: function jsBoostWidgetsmodal(), received:", args.command);
        }
    }
    ...
    function blockScreenClose() {
        jsBoostWidgets("#divBlockScreen").hidden();
        jsBoostWidgets("body").remove("#divBlockScreen");
    }
    ...
    function tooltip() {
        ...
        let _timeout_curr  = "";
        let _close_content = '\
        <a id="a_content" onclick="return  jsBoostWidgets(\'#div_tooltip\').hidden();">\
        ...
        jsBoostWidgets("#div_close_tooltip").html(_close_content);
        ...
        setTimeout(function(){
            ...
            jsBoostWidgets("#div_tooltip").fadeIn();

            clearTimeout( _timeout_curr );
            
            jsBoostWidgets("#div_tooltip_content").html(_message);

            _timeout_curr = setTimeout(function(){
                clearTimeout( _timeout_curr );
                jsBoostWidgets("#div_tooltip").fadeOut();
            }, _timeout );
    ...
    function slider(params) {
        ...
        let interval = setInterval(function() {
            ...
            // Slider Image View
            jsBoostWidgets(selector).html('<img src="'+imagelist[n]+'"" id="image_'+n+'" />');
            // For Fade Effect
            if(params.mode == "fade") {
                jsBoostWidgets(selector).fadeIn(parseInt(params.timefade));
            }
            // For Slide Effect
            if(params.mode == "slide") {
                jsBoostWidgets(selector).slideIn('#image_'+n);
            }
            // For Mixer Effect
            if(params.mode == "mix") {
                jsBoostWidgets(selector).mixIn(parseInt(params.timefade));
            }
            n+=1; i+=1;
